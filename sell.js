...
}else if (type === 'waxSummary') {
  document.getElementById('waxSummaryModal').classList.remove('hidden');
}else if (type === 'chalkProductSummary') {
  document.getElementById('chalkProductSummaryModal').classList.remove('hidden');
}
...
async function showChalkProductSummary() {
  const summary = {};
  const snapshot = await db.collection('chalkSales').get();

  snapshot.forEach(doc => {
    const item = doc.data();
    const product = item?.product?.trim() || 'Unknown';

    if (!summary[product]) {
      summary[product] = { qty: 0, profit: 0 };
    }

    summary[product].qty += item.qty || 0;
    summary[product].profit += item.profit || 0;
  });
  ...
  let html = `
    <table class="w-full border border-gray-600 text-sm text-center">
      <thead class="bg-gray-700 text-gray-300">
        <tr>
          <th class="p-2 border border-gray-600">Product</th>
  ...
  Object.entries(summary).forEach(([product, data]) => {
    html += `
      <tr class="border-b border-gray-700">
        <td class="p-2 border border-gray-600">${product}</td>
  ...
  html += `</tbody></table>`;
  document.getElementById('chalkProductSummaryContent').innerHTML = html;
}
...
async function showWaxSummary() {
  const summary = {};
  const snapshot = await db.collection('waxSales').get();

  snapshot.forEach(doc => {
    const item = doc.data();
    const unit = item?.unit?.trim() || 'Unknown';

    if (!summary[unit]) {
      summary[unit] = { qty: 0, profit: 0 };
    }

    summary[unit].qty += item.qty || 0;
    summary[unit].profit += item.profit || 0;
  });

  let html = `
    <h2 class="text-xl font-bold text-yellow-400 mb-2">📊 Wax Unit Summary</h2>
    <table class="w-full border border-gray-600 text-sm text-center">
      <thead class="bg-gray-700 text-gray-300">
        <tr>
          <th class="p-2 border border-gray-600">Unit</th>
          <th class="p-2 border border-gray-600">Total Qty</th>
          <th class="p-2 border border-gray-600">Total Profit (৳)</th>
        </tr>
      </thead>
  ...
  Object.entries(summary).forEach(([unit, data]) => {
    html += `
      <tr class="border-b border-gray-700">
        <td class="p-2 border border-gray-600">${unit}</td>
        <td class="p-2 border border-gray-600">${data.qty}</td>
        <td class="p-2 border border-gray-600">${data.profit.toFixed(2)}</td>
      </tr>
    `;
  ...
  html += `</tbody></table>`;
  document.getElementById('waxSummaryContent').innerHTML = html;
}
...
async function showChalkSummary() {
  const summary = {};
  const snapshot = await db.collection('chalkSales').get();

  snapshot.forEach(doc => {
    const item = doc.data();
    const size = item?.size?.trim() || 'Unknown';

    if (!summary[size]) {
      summary[size] = { qty: 0, profit: 0 };
    }

    summary[size].qty += item.qty || 0;
    summary[size].profit += item.profit || 0;
  });
...
function renderShopListTable() {
  ...
  shops.forEach((shop, index) => {
    ...
    tr.innerHTML = `
      ...
      ${shop.lat && shop.lng ? `<button onclick="window.open('https://www.google.com/maps?q=${shop.lat},${shop.lng}')" class="text-blue-400 hover:text-blue-200 ml-2">📍Map</button>` : ''}
      </td>
    `;
...
function submitForm(type) {
  ...
  // ✅ Tom Select থেকে shop নেওয়া
  let shopSelectId = type === 'wax' ? '#waxShopSelect' : '#chalkShopSelect';
  let shopId = document.querySelector(shopSelectId)?.tomselect?.getValue();
  let shopName = document.querySelector(shopSelectId)?.tomselect?.getItem(shopId)?.textContent?.trim();

  const formData = {
    date: document.getElementById(`${type}Date`).value.trim(),
    shop: shopName,
    qty: parseFloat(document.getElementById(`${type}Qty`).value) || 0,
    rate: parseFloat(document.getElementById(`${type}Rate`).value) || 0,
    amount: parseFloat(document.getElementById(`${type}Amount`).value) || 0,
    payment: parseFloat(document.getElementById(`${type}Payment`).value) || 0,
    due: parseFloat(document.getElementById(`${type}Due`).value) || 0,
    profit: parseFloat(document.getElementById(`${type}ProfitInput`).value) || 0,
    type: type
  };

  // ✅ Wax হলে unit ধরছি
  if (isWax) {
    const unitVal = document.getElementById('waxUnit')?.value?.trim();
    formData.unit = unitVal;
  }

  // ✅ Chalk হলে size আর product ধরছি
  if (!isWax) {
    const sizeVal = document.getElementById('chalkSize').value.trim();
    const productVal = document.getElementById('chalkProduct')?.value?.trim();
    formData.size = sizeVal;
    formData.product = productVal;
  }

  // ✅ Validation
  if (
    !formData.date || !formData.shop || formData.qty <= 0 || formData.rate <= 0 ||
    formData.amount <= 0 || formData.payment < 0 || formData.due < 0 || formData.profit < 0 ||
    (!isWax && (!formData.size || !formData.product)) || (isWax && !formData.unit)
  ) {
...
async function editEntry(type, index) {
  ...
  // 🔍 Firestore থেকে matching document ID খুঁজে বের করো
  await db.collection(type + 'Sales').get().then(snapshot => {
    snapshot.forEach(doc => {
      ...
      if (isSame) matchedDocId = doc.id;
    });
  });

  if (!matchedDocId) {
    showToast("❌ Could not find the original entry", "error");
    return;
  }
  ...
  // ✅ ফর্ম প্রিফিল করার জন্য delay দাও
  setTimeout(() => {
    // 📅 Common Fields
    document.getElementById(type + 'Date').value = item.date;
    document.getElementById(type + 'Qty').value = item.qty;
    document.getElementById(type + 'Rate').value = item.rate;
    document.getElementById(type + 'Amount').value = item.amount;
    document.getElementById(type + 'Payment').value = item.payment;
    document.getElementById(type + 'Due').value = item.due;
    document.getElementById(type + 'ProfitInput').value = item.profit;

    // 🏪 Shop Select (TomSelect)
    const shopSelect = document.getElementById(type + 'ShopSelect');
    if (shopSelect?.tomselect) {
      shopSelect.tomselect.setValue(item.shop);
    }

    // 📏 Size Select (only for chalk)
    if (type === 'chalk') {
      const sizeSelect = document.getElementById('chalkSize');
      if (sizeSelect?.tomselect) {
        sizeSelect.tomselect.setValue(item.size);
      }
    }

    // 🔄 Local থেকে remove (update হলে আবার push হবে)
    data.splice(index, 1);
    saveData(type);
  }, 100); // ⏱️ Delay to ensure DOM fully loaded
...
const lat = parseFloat(document.getElementById('shopLatInput').value);
const lng = parseFloat(document.getElementById('shopLngInput').value);
...
const shopData = { name, address, mobile, lat, lng };
...
function getCurrentLocation() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      document.getElementById('shopLatInput').value = position.coords.latitude;
      document.getElementById('shopLngInput').value = position.coords.longitude;
    }, () => {
...
window.onload = function () {
  if (typeof init === 'function') init();         // 🛠️ যদি init থাকে
  if (typeof renderExpenses === 'function') renderExpenses(); // 💸 খরচ দেখানোর ফাংশন
  if (typeof loadShopsFromFirebase === 'function') loadShopsFromFirebase(); // 🏪 দোকান লোড
  if (typeof showChalkSummary === 'function') showChalkSummary(); // 📊 আমাদের নতুন summary
  if (typeof showWaxSummary === 'function') showWaxSummary(); // ✅ Add this
  if (typeof showChalkProductSummary === 'function') showChalkProductSummary(); // ✅ নতুন
  window.nextDocToUpdate = null;
};
